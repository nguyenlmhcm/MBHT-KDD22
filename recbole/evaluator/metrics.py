...
class MRR(TopkMetric):
    ...
    def metric_info(self, pos_index):
        ...
        result = np.zeros_like(pos_index, dtype=float)  # ENV-COMPAT: np.float removed in modern numpy
...
class MAP(TopkMetric):
    ...
    def metric_info(self, pos_index, pos_len):
        ...
        sum_pre = np.cumsum(pre * pos_index.astype(float), axis=1)  # ENV-COMPAT: np.float removed in modern numpy
...
class NDCG(TopkMetric):
    ...
    def metric_info(self, pos_index, pos_len):
        ...
        iranks = np.zeros_like(pos_index, dtype=float)  # ENV-COMPAT: np.float removed in modern numpy
        iranks[:, :] = np.arange(1, pos_index.shape[1] + 1)
        idcg = np.cumsum(1.0 / np.log2(iranks + 1), axis=1)
        for row, idx in enumerate(idcg_len):
            idcg[row, idx:] = idcg[row, idx - 1]

        ranks = np.zeros_like(pos_index, dtype=float)  # ENV-COMPAT: np.float removed in modern numpy
...
class GAUC(AbstractMetric):
    ...
    def metric_info(self, pos_rank_sum, user_len_list, pos_len_list):
        ...
        non_zero_idx = np.full(len(user_len_list), True, dtype=bool)  # ENV-COMPAT: np.bool removed in modern numpy
        if any_without_pos:
            logger = getLogger()
            logger.warning(
